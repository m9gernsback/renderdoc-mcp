...
void registerSessionTools(ToolRegistry& registry) {
    registry.registerTool({
        ...
        "All tools work over remote replay including get_shader, run_shader_tool, get_pipeline_state. "
        "IMPORTANT: For remote replay, always call goto_event(eventId) FIRST, then query shader/pipeline "
        "at the SAME eventId. This ensures only one remote proxy call per event navigation. "
        "Changing to a different eventId triggers another proxy call. "
        "Setup steps:\n"
